...
class TermsOfServiceController extends GetxController {
  ...
  List<TermsOfServiceData> termsOfServiceDataList = [];
  Future<UiResult<bool>> termsOfService() async {
    try {
      isLoading = true;
      EasyLoading.show();
      final response = await userRepo.termsOfService();
      termsOfServiceDataList = response;
  ...
  List<TermsOfServiceData> privacyPolicyDataList = [];
  Future<UiResult<bool>> privacyPolicy() async {
    try {
      isLoading = true;
      EasyLoading.show();
      final response = await userRepo.privacyPolicy();
      privacyPolicyDataList = response;
      return const UiSuccess(true);
    } catch (error, stackTrace) {
      return ErrorUtil.getUiFailureFromException(error, stackTrace);
    } finally {
